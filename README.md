...
页面右上角和页脚会同时显示应用版本号。应用版本与 PUBG 游戏版本相互独立，并采用语义化版本格式 `主版本.功能版本.修订版本`：

- 不兼容的结构或算法重做：升级主版本，例如 `v2.0.0`。
- 新增测试、选项或生成能力：升级功能版本，例如 `v1.3.0`。
- 文案、样式或问题修复：升级修订版本，例如 `v1.2.1`。

每次发布页面更新时，必须同步修改独立 HTML、`package.json`、`package-lock.json` 和 README 中的版本号。
...
- 个性化生成：综合 DPI、当前常规灵敏度、瞄准风格、压枪发力方式与测试表现计算参数。
- 发力方式适配：提供手指、手腕和手臂三种压枪曲线，分别调整 ADS、倍镜、垂直倍率与推荐 `cm/360`。
- 转身手感校正：感觉转身偏慢或偏快时，单独调整常规与肩射灵敏度，不破坏 ADS 和倍镜压枪参数。
...
如果你习惯用较小的手指位移压枪，请选择“手指”。该模式会提高 ADS、近中倍镜和垂直倍率，避免低灵敏度迫使玩家使用整个手臂下拉。习惯大范围稳定移动的玩家可以选择“手臂”，介于两者之间则选择“手腕”。
